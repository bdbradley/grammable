...
RSpec.describe GramsController, type: :controller do
  ...
  describe "grams#update action" do
    ...
    it "shouldn't let users who didn't create the gram update it" do
      #creates user assigned to a gram
      gram = FactoryBot.create(:gram)
      #creates another user (with no gram)
      user = FactoryBot.create(:user)
      #that user tries to sign in and update gram
      sign_in user
      #runs a HTTP patch request to update the gram
      patch :update, params: { id: gram.id, gram: { message: 'wahoo' } }
      #expects response to be that it will fail w/ status of forbidden
      expect(response).to have_http_status(:forbidden)
    end
    ...
    it "should allow users to successfully update grams" do
      ...
      sign_in gram.user
      #HTTP patch request to update action, patch#update
    ...
    it "should render the edit form with an http status of unprocessable_entity" do
      gram = FactoryBot.create(:gram, message: "Initial Value")
      #user that created the gram is the user that is signed in
      sign_in gram.user
